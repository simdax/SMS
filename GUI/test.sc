...
/*
	...
	w=Window("window title", Rect(0, 0, 400, 400)).alwaysOnTop_(true).front.acceptsMouseOver_(true)
;
	w.addFlowLayout;
	MyWidget(w, Rect(0,0,100,100)).valueAction_(0.5).refresh;
	PopUpMenu(w, Rect(0,0,100,100)).valueAction_(0.5)
...
MyWidget : UserView {
	...
	*new{ |argParent, argBounds|
		^super.new(argParent, argBounds).init;
	}
	init{
	...
	mouseMove{ arg x, y, modifiers, buttonNumber, clickCount;
		...
		([256, 0].includes(modifiers)).if{ // restrict to no modifier
			...
			newVal= x.linlin(0,this.bounds.width,0,1);
			// translates therelative mouse position in pixels to a value between 0 and 1
